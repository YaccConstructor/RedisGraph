...
GrB_Index I_cpu3[30000000];
GrB_Index J_cpu3[30000000];
...
int CFPQ_cpu3(RedisModuleCtx *ctx, GraphContext *gc, bool_automat *grammar, CfpqResponse *response)
{
    ...
    // create graph matrix
    uint64_t count_matrices = GraphContext_SchemaCount(gc, SCHEMA_EDGE);
    uint64_t sizeGraph = Graph_RequiredMatrixDim(gc->g);
    GrB_Matrix matrices[50];
	...
	int offset = 0;
    int count_matrices_real = 0;
    while (offset < count_matrices)
    {
        char *label = gc->relation_schemas[offset]->name;
        ...
        if (label_id != INT_MIN)
        {
            printf("%s \n", label);
            uint64_t index = 0;
            while (grammar->id[index] != label_id)
            {
                index++;
            }

            GrB_Matrix_dup(&matrices[index], gc->g->relations[offset]);
            count_matrices_real++;
        }
...
// create bool semiring
    ...
    while(matrices_is_changed)
    {
        ...
        for (uint64_t i = 0; i < count_matrices_real; i++)
        {
        ...
        for (int n = 0; n < count_S_term; n++)
        {
            ...
            {
                bool st = false;
                GrB_Matrix_extractElement_BOOL(&st, States[n], 0, i);

                if (st)
                {
                    for (uint64_t k = 0; k < sizeGraph; k++)
                    {
                        I_cpu3[k] = 0 * sizeGraph + k;
                        J_cpu3[k] = i * sizeGraph + k;
                    }

                    GrB_Matrix_extract(block, NULL, NULL, Kproduct, I_cpu3, sizeGraph, J_cpu3, sizeGraph, NULL);
					int64_t id = grammar->id[count_matrices_real + n];
                    int index = 5;
