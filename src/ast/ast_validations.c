...
static void _AST_GetIdentifiers(const cypher_astnode_t *node, rax *identifiers) {
	...
	if(cypher_astnode_type(node) == CYPHER_AST_PATH_PATTERN_REFERENCE) return;
...
static void _AST_GetReferredIdentifiers(const cypher_astnode_t *node, rax *identifiers) {
	...
	} else if (cypher_astnode_type(node) == CYPHER_AST_NAMED_PATH) {
	    // Dont`t collect identifiers in named path patterns
        return;
    } else {
