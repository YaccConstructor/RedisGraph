...
static void _QueryGraphAddPathPattern(QueryGraph *qg,
...
static void _QueryGraph_ExtractPathPattern(const QueryGraph *qg, QueryGraph *graph,
									QGNode *left, QGNode *right, AST *ast, const cypher_astnode_t *ast_path_pattern) {
...
static void _QueryGraph_ExtractPath(const QueryGraph *qg, QueryGraph *graph,
	...
	/* Introduce edges and path patterns to graph
	 * they are at odd indices */
	for(uint i = 1; i < nelems; i += 2) {
        ...
        if(cypher_astnode_type(ast_node) == CYPHER_AST_REL_PATTERN) {
		    _QueryGraph_ExtractEdge(qg, graph, left, right, ast, ast_node);
        }
        else {
            _QueryGraph_ExtractPathPattern(qg, graph, left, right, ast, ast_node);
        }
...
void QueryGraph_AddPath(QueryGraph *qg, const cypher_astnode_t *path) {
	...
	for(uint i = 1; i < nelems; i += 2) {
        ...
        if (cypher_astnode_instanceof(edge, CYPHER_AST_REL_PATTERN)) {
            ...
            _QueryGraphAddPathPattern(qg, edge, left, right);
        } else {
            assert(false);
        }
	}
