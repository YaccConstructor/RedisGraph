  ...
  gpuErrchk(cudaMalloc(&d_ppcPtr, sizeof(TYPE *) * num_tables))
    ...
    gpuErrchk(cudaMalloc(&d_pcPtr, sizeof(TYPE) * num_rows * num_cols))
        gpuErrchk(cudaMemset(d_pcPtr, 0, sizeof(TYPE) * num_rows * num_cols))
            gpuErrchk(cudaMemcpy(&d_ppcPtr[i], &d_pcPtr, sizeof(TYPE *),
...
void delete_tables(TYPE **tables, int num_tables) {
  ...
  gpuErrchk(cudaMallocHost((void **)&someHost, sizeof(TYPE *) * num_tables))
      gpuErrchk(cudaMemcpy(someHost, tables, num_tables * sizeof(TYPE *),
                           cudaMemcpyDeviceToHost))

          for (int i = 0; i < num_tables; i++){gpuErrchk(
              cudaFree((void *)someHost[i]))} gpuErrchk(cudaFree(tables))
              gpuErrchk(cudaFreeHost(someHost))
}

TYPE *allocate_matrix_host(int rows, int cols) {
  TYPE *matrix;
  gpuErrchk(cudaMallocHost((void **)&matrix,
                           sizeof(TYPE) * rows * cols)) return matrix;
}

void delete_matrix_host(TYPE *matrix){gpuErrchk(cudaFreeHost(matrix))}

TYPE *allocate_matrix_device(int rows, int cols) {
  TYPE *matrix;
  gpuErrchk(
      cudaMalloc((void **)&matrix, sizeof(TYPE) * rows * cols)) return matrix;
}

void delete_matrix_device(TYPE *matrix) { gpuErrchk(cudaFree(matrix)) }

void copy_device_to_host_sync(TYPE *device, TYPE *host, int elems) {
  gpuErrchk(
      cudaMemcpy(host, device, sizeof(TYPE) * elems, cudaMemcpyDeviceToHost))
}

void copy_host_to_device_sync(TYPE *host, TYPE *device, int elems) {
  gpuErrchk(
      cudaMemcpy(device, host, sizeof(TYPE) * elems, cudaMemcpyHostToDevice))
}

void copy_device_to_device_sync(TYPE *src, TYPE *dst, int elems) {
  gpuErrchk(
      cudaMemcpy(dst, src, sizeof(TYPE) * elems, cudaMemcpyDeviceToDevice))
}

void copy_device_to_host_async(TYPE *device, TYPE *host, int elems) {
  gpuErrchk(cudaMemcpyAsync(host, device, sizeof(TYPE) * elems,
                            cudaMemcpyDeviceToHost))
}

void copy_host_to_device_async(TYPE *host, TYPE *device, int elems) {
  gpuErrchk(cudaMemcpyAsync(device, host, sizeof(TYPE) * elems,
                            cudaMemcpyHostToDevice))
}

void synchronize_with_gpu() { cudaDeviceSynchronize(); }
