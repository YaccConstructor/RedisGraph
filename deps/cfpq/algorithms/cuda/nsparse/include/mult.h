...
const matrix<bool, index_type> spgemm(const matrix<bool, index_type> &c,
  ...
  thrust::device_vector<index_type> row_index =
      calc_nz_per_row(n_rows, n_cols, c.m_col_index, c.m_row_index, a.m_col_index,
                      a.m_row_index, b.m_col_index, b.m_row_index);

  thrust::device_vector<index_type> col_index =
      fill_nz_per_row(n_rows, c.m_col_index, c.m_row_index, a.m_col_index,
                      a.m_row_index, b.m_col_index, b.m_row_index, row_index);

  assert(row_index.size() == n_rows + 1);
  assert(col_index.size() == row_index.back());
