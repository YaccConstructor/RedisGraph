...
void exec_merge_count(const thrust::device_vector<index_type>& rpt_a,
  ...
  EXPAND_SIDE_EFFECTS((
      bin_size[Borders::bin_index] > 0 ? merge_path_count<index_type, Borders::config_t::block_size>
...
void exec_merge_fill(const thrust::device_vector<index_type>& rpt_a,
                     ...
                     const thrust::device_vector<index_type>& bin_size, std::tuple<Borders...>) {
  EXPAND_SIDE_EFFECTS(
      (bin_size[Borders::bin_index] > 0 ? merge_path_fill<index_type, Borders::config_t::block_size>
           <<<(index_type)bin_size[Borders::bin_index], Borders::config_t::block_size>>>(
...
std::pair<thrust::device_vector<index_type>, thrust::device_vector<index_type>> unique_merge(
    ...
    const thrust::device_vector<index_type>& rpt_b, const thrust::device_vector<index_type>& col_b,
    std::tuple<Borders...>) {
  ...
  thrust::device_vector<index_type> permutation_buffer(rows);
