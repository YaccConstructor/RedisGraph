...
void exec_block_row(const thrust::device_vector<index_type>& c_col_idx,
  ...
  EXPAND_SIDE_EFFECTS((bin_size[Borders::bin_index] > 0
                           ? fill_nz_block_row<index_type, Borders::max_border>
...
template <typename index_type, typename... Borders>
thrust::device_vector<index_type> fill_nz_per_row(
...
void reuse_global_hash_table(const thrust::device_vector<index_type>& row_idx,
                             thrust::device_vector<index_type>& col_idx,
                             const global_hash_table_state_t<index_type>& state) {
  constexpr index_type block_sz = 1024;
